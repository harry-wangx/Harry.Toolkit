    ...
    public class ConcurrentQueue<T> : IProducerConsumerCollection<T>//, IReadOnlyCollection<T>
    {
        ...
        private class Segment
        {
            ...
            internal void UnsafeAdd(T value)
            {
                ...
                {
                    throw new Exception("m_high值不正确");
                }
            ...
            internal void Grow()
            {
                ...
                if (m_source.m_tail != this)
                {
                    throw new Exception("m_source.m_tail != this");
                }
                ...
                while (lowLocal <= highLocal)
                {
                    ...
                    if (Interlocked.CompareExchange(ref m_low, lowLocal + 1, lowLocal) == lowLocal)
                    {
                        ...
                        if (lowLocal + 1 >= SEGMENT_SIZE)
                        {
                            ...
                            if (m_source.m_head != this)
                            {
                                throw new Exception("m_source.m_head != this");
                            }
